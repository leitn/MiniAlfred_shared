...
/*   Updated: 2023/11/22 21:52:20 by hedubois         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
void	check_fds(t_shell *shell, t_elem *tmp)
{
	int i;
	...
	{
		if (i == 0 && check_bltn(tmp) == true)
			tmp->fd_wr = shell->pipe[i][1]; // it says 4
		else if (i == shell->tree->count_pipe && check_bltn(tmp) == true)
			tmp->fd_rd = shell->pipe[i - 1][0];
		else if (i > 0 && check_bltn(tmp) == true)
		{
			tmp->fd_wr = shell->pipe[i][1];
			tmp->fd_rd = shell->pipe[i - 1][0];
		}
		if (tmp->after == PIPE)
			tmp = tmp->next;
...
int	ft_wait_children(t_shell *shell)
{
	...
	waitpid(shell->pids[shell->tree->count_pipe], &error_status, 0);
